...
2. **Write each draft into its own file** under `tasks/<client>/active/<task-id>-slack-reply-N.txt` (incrementing `N`). Use `.txt`, not `.md`, because rendered markdown strips the mrkdwn characters when copied. Keep each file single-purpose: one question from the colleague → one reply file.

3. **Format for Slack mrkdwn**, not CommonMark:
...
4. **Match the colleague's language register.** If they write in Serbian (ekavica), reply in Croatian (ijekavica) only if the user explicitly asks; otherwise mirror the colleague.

5. **Length is dictated by the question.** A "how do I trigger this manually" question gets 3–5 lines; an architectural pitch gets the full structure (problem → approach → example output → prerequisites → implementation plan). Do not default to long.

6. **When the colleague pushes code, read it before answering.** Clone or `git fetch` the branch they mention, read the action/workflow, and cite line-level issues (hardcoded values, branch-matching bugs, missing inputs) in the reply.

7. **Do not invent.** If the user says "imamo `GIT_PAT`", trust it. Don't re-derive or propose creating new secrets when existing ones cover the case. If you're not sure whether a secret/webhook/app exists, say so and ask rather than speculating.

8. **Accept being wrong.** The user will push back ("jesi sig?", "koja je bila prva iteracija") — treat that as a prompt to re-check, not to defend. If you change your mind after reading new context, state that explicitly ("prva iteracija bila je X, nakon što sam pročitao Y sam pivotirao na Z").
